...
void BufferPoolManagerInstance::FlushAllPgsImp() {
  // You can do it!
  std::scoped_lock<std::mutex> lock(latch_);
  for (const auto &page2frame : page_table_) {
    FlushPgImp(page2frame.first);
  }
}
...
auto BufferPoolManagerInstance::FindPage(page_id_t page_id) -> frame_id_t {
  auto iter = page_table_.find(page_id);
  if (iter != page_table_.end()) {
    return iter->second;
  }
...
auto BufferPoolManagerInstance::FindAvailablePg() -> frame_id_t {
  frame_id_t ans;
  if (!free_list_.empty()) {
